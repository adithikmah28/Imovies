document.addEventListener('DOMContentLoaded', () => {
    ...
    async function displaySeriesRow(endpoint, container) {
        ...
        try {
            ...
            container.innerHTML = '';
            if (data.results.length === 0) { container.innerHTML = '<p style="color: #888;">No series found.</p>'; return; }
            data.results.forEach(series => {
                if (!series.poster_path) return;
                const seriesLink = document.createElement('a');
                seriesLink.href = `/series/${series.id}`; // Link ke detail series
    ...
    async function performSearch(query) {
        ...
        const endpoint = `/search/tv?query=${encodeURIComponent(query)}`; // Endpoint search TV
    ...
    async function initializePage() {
        const categories = {
            ...
            anime: { endpoint: '/discover/tv?with_genres=16&with_keywords=210024|287501', selector: '#anime-series .movie-list' } // Genre Animasi & keyword Anime/Japanimation
        };
